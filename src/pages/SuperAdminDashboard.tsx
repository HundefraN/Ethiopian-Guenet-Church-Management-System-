...
export default function SuperAdminDashboard() {
  ...
  const handleToggleMaintenanceClick = () => {
    ...
    setConfirmTitle(newValue ? "Enable Maintenance Mode" : "Disable Maintenance Mode");
    setConfirmMessage(newValue
      ? "Are you sure you want to enable maintenance mode? Users will not be able to access the system."
      : "Are you sure you want to disable maintenance mode? The system will be accessible to all users.");
    setConfirmType("warning");
    setConfirmButtonText(newValue ? "Enable" : "Disable");
  ...
  const toggleMaintenance = async () => {
    ...
    if (!error) {
      ...
      toast.success(`Maintenance mode ${newValue ? "enabled" : "disabled"}`, { id: loadingToast });
    }
    else { toast.error("Failed to update settings", { id: loadingToast }); }
  ...
  return (
    <motion.div initial="hidden" animate="visible" variants={containerVariants} className="space-y-8 max-w-7xl mx-auto pb-12">
        ...
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div>
            <div className="flex items-center gap-3 mb-3">
              <div className="flex items-center gap-2 bg-white/10 backdrop-blur-sm border border-white/20 px-3 py-1.5 rounded-full">
                ...
                <span className="text-emerald-300 text-xs font-bold uppercase tracking-wider">System Online</span>
              </div>
            </div>
            ...
            <p className="text-blue-200/80 font-medium max-w-lg text-sm">
              Real-time system overview, global analytics, and administrative controls for the entire church network.
            </p>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            >
              ...
              <span>{maintenanceMode ? "Maintenance ON" : "System Active"}</span>
            </button>
          </div>
        </div>
      </motion.div>
      ...
      <motion.div variants={containerVariants} className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-5">
        {[
          { label: "Branches", value: stats.churches, recent: stats.recentChurches, icon: Building, gradient: "from-blue-600 to-blue-400", bgLight: "bg-blue-50", textColor: "text-blue-600" },
          { label: "Pastors", value: stats.pastors, recent: 0, icon: Shield, gradient: "from-violet-600 to-purple-400", bgLight: "bg-purple-50", textColor: "text-purple-600" },
          { label: "Servants", value: stats.servants, recent: 0, icon: UserCheck, gradient: "from-amber-500 to-orange-400", bgLight: "bg-orange-50", textColor: "text-orange-600" },
      ...
      <motion.div variants={containerVariants} className="grid grid-cols-1 lg:grid-cols-5 gap-5">
        ...
        <motion.div variants={itemVariants} className="lg:col-span-2 bg-white rounded-[1.5rem] p-6 md:p-8 border border-gray-100/80 shadow-[0_4px_24px_rgba(0,0,0,0.04)]">
          ...
          <div className="w-44 h-44 mx-auto mb-6">
            <DonutChart segments={[
              { value: stats.pastors, color: '#8b5cf6', label: 'Pastors' },
              { value: stats.servants, color: '#f59e0b', label: 'Servants' },
              { value: stats.members, color: '#10b981', label: 'Members' },
            ]} />
          </div>
          <div className="space-y-2.5">
            {[
              { label: 'Pastors', value: stats.pastors, color: 'bg-violet-500', pct: totalPeople > 0 ? Math.round((stats.pastors / totalPeople) * 100) : 0 },
              { label: 'Servants', value: stats.servants, color: 'bg-amber-500', pct: totalPeople > 0 ? Math.round((stats.servants / totalPeople) * 100) : 0 },
              { label: 'Members', value: stats.members, color: 'bg-emerald-500', pct: totalPeople > 0 ? Math.round((stats.members / totalPeople) * 100) : 0 },
            ].map((item, i) => (
          ...
          <div className="space-y-7">
            ...
            <div>
              ...
              <div className="flex gap-3">
                {[
                  { label: "Active", value: stats.activeMembers, color: "from-emerald-400 to-emerald-500", bg: "bg-emerald-50", text: "text-emerald-700" },
          ...
          <div className="p-8 relative z-10">
            <div className="flex items-center gap-3 mb-8">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 font-medium">Global system metrics</p>
              </div>
            </div>
            ...
            <div className="mb-8">
              <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6 text-center">
                ...
                <span className="text-xs font-bold text-blue-300/80 uppercase tracking-wider">Total Registered Accounts</span>
              </div>
            </div>

            <div className="space-y-4">
              {[
                { label: 'Servant to Member Ratio', value: stats.servants > 0 ? `1:${Math.round(stats.members / stats.servants)}` : 'N/A', icon: UserCheck },
                { label: 'Avg Members / Branch', value: stats.churches > 0 ? Math.round(stats.members / stats.churches) : 0, icon: Building },
